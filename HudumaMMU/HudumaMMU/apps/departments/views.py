...
class DepartmentAPIView(generics.CreateAPIView):
    ...
    def post(self, request):
        id_ = request.data.get("department_id", '')
        queryset = Department.objects.filter(id=id_).first()
        serializer = DepartmentSerializers(queryset, context={'request': request})
        return Response(serializer.data, status=status.HTTP_200_OK)
    
    def patch(self, request):
        """update department"""
        id_ = request.data.pop("department_id", '')
        queryset = Department.objects.filter(id=id_).first()
        department_data = request.data
        serializer = self.serializer_class(instance = queryset,
            data=department_data, partial=True)
